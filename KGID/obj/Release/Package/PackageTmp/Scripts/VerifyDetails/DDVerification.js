...
$(document).ready(function () {
    ...
    if (sanctionedAmount > 2500000.00) {
        $("#btnDDSubmit").text("Send");
        $("#divApprove").hide();
        $("#divHealthOpinion1").show();
    }
    else if (sanctionedAmount > 2000000.00) {
        $("#btnDDSubmit").text("Send");
        $("#divApprove").hide();
    }
    else {
        $("#btnDDSubmit").text("Accept");
        $("#divApprove").show();
    }
...
$("#btnDDSubmit").click(function (e) {
    ...
    else if ($("input[name='ApplicationStatus']:checked").val() === "13")
        qnText = "Are you sure you want to forward application to Director?"
    else if ($("input[name='ApplicationStatus']:checked").val() === "16")
    ...
    if ($("input[name='ApplicationStatus']:checked").val() === "10")
        ...
        respText = "Application forwarded to Director"
    else if ($("input[name='ApplicationStatus']:checked").val() === "16")
        respText = "Application sent for health opinion"

    if (isStatusSelected && isCheckboxchecked && isSendBack) {
        e.preventDefault();
        alertify.confirm(qnText, function () {
            //if (btnresult == true) {
                var caseWorkerVerifiedDetails = new FormData($("#frmDDVerify").get(0));
                //e.preventDefault();
...
$("input[name='ApplicationStatus']").change(function () {
    ...
    if ($("input[name='ApplicationStatus']:checked").val() === "10" || $("input[name='ApplicationStatus']:checked").val() === "13" || $("input[name='ApplicationStatus']:checked").val() === "16") {
        $("#btnDDSubmit").text("Send");
    }
    else {
        $("#btnDDSubmit").text("Accept");
...
function DocFileChange(id, errLbl) {
    ...
    if ($("#" + e.target.id).get(0).files[0] != undefined) {
        const fsize = $("#" + e.target.id).get(0).files[0].size;
...
$("#btnDDClear").click(function (e) {
    $("#ddlRemarks").val("");
    $("#txtComments").val("");
    $("#txtDLoadFactor").val("");
    $("#rdBtnForwardToD").prop("checked", false);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if ($("#ddlRemarks option:selected").text() == "No Correction Found") {
                ...
                //$("#divHealthOpinion1").show();
                $("#divApprove").hide();
                if ((parseFloat($("#txtSumAssured").val()) > parseFloat(2500000.00)) && $("#lblDHSOpinion").text() !== "" && $("#lblDHSOpinion").text() !== undefined) {
                    $("#divApprove").hide();
                    $("#divForwardToDD").show();
                }
                else if ((parseFloat($("#txtSumAssured").val()) <= parseFloat(2500000.00))) {
                    $("#divApprove").show();
                    $("#divForwardToDD").show();
                }
                else if ((parseFloat($("#txtSumAssured").val()) > parseFloat(2500000.00)) && ($("#lblDHSOpinion").text() === "" || $("#lblDHSOpinion").text() === undefined)) {
                    $("#divApprove").hide();
                    $("#divForwardToDD").hide();
                }
                //if (parseFloat($("#txtSumAssured").val()) <= 2000000.00) {
                //    $("#divApprove").show();
                //}
            } else {
                $("#lblSendBackDIO").show();
...
function showHealthOpinion() {
    if ($("div:contains('Previously uploaded Document(DHS).')").length > 0) {
        $("#divHealthOpinion1").hide();
    } else {
        $("#divHealthOpinion1").show();
    }
