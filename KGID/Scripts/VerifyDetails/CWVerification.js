...
$("#btnSubmit").click(function (e) {
    ...
    if ($("#hdnVerificationType").val() === "NHO") {
        $("#hdnHealthOpinion").val(true);
        $("#hdnHOApplicationStatus").val(9);
        $("#hdnVerifyProposerDetails").val(true);
        if ($("#flupldCWHealthOpinion").val().length == 0) {
        ...
        if (isDocUploaded) {
            alertify.confirm("Are you sure you want to submit DHO opinion?", function () {
        ...
        if (isStatusSelected && isCheckboxchecked && isDocUploaded) {
            alertify.confirm("Are you sure you want to forward application to Superintendent?", function () {
                //$("#frmCWVerDetails").submit();
                var caseWorkerVerifiedDetails = new FormData($("#frmCWVerDetails").get(0));
                $.ajax({
                    type: 'POST',
                    url: '/SaveCWVData/',
                    data: caseWorkerVerifiedDetails,
                    ...
                    success: function (result) {
                        if (result == "1") {
                            alertify.alert("Forwarded to Superintendent.", function () {
                                window.location.href = "/kgid-cw/";
                            }).setHeader("Attention");
                        }
                        else {
                            alertify.alert("There is no Superintendent mapped for this district.", function () {
                                window.location.href = "/kgid-cw/";
                            }).setHeader("Warning");
                        }
                    }
...
$("#btnClear").click(function (e) {
    $("#ddlRemarks").val("");
    $("#txtComments").val("");
    $("#rdBtnForwardToSup").prop("checked", false);
