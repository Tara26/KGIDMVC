...
$("#btnUpdateDetails").click(function () {
    ...
    if ($("#txtKGIDNO").val() == "") { $("#txtKGIDNO").val("0"); }
    $(".err").attr("hidden", true);
    ...
    if ($(".err:visible").length == 0) {
        ...
        $.ajax({
            ...
            data: { KGIDNumber: $("#txtKGIDNO").val(), MobileNumber: $("#txtMBLNO").val(), EMailID: $("#txtEmailID").val(), EmployeeId: $("#hdnEmpId").val() },
            success: function (result) {
                if (result == "2") {
                    // alertify.warning("Provided Email ID and Mobile number already exists");
                    alertify.warning("Provided details already exists");
                }
                else if (result == "3") {
